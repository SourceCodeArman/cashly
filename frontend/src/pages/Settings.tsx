...
export function Settings() {
  ...
  // Tab tracking for directional animations
  const tabOrder = ['profile', 'accounts', 'security', 'preferences'] as const
  type TabValue = typeof tabOrder[number]

  const [currentTab, setCurrentTab] = useState<TabValue>('profile')
  const [slideDirection, setSlideDirection] = useState<'left' | 'right'>('right')

  // Handle tab changes with directional animation
  const handleTabChange = (newTab: string) => {
    const oldIndex = tabOrder.indexOf(currentTab)
    const newIndex = tabOrder.indexOf(newTab as TabValue)

    // Determine slide direction based on tab order
    // Moving forward (right) in tabs: new content slides in from right
    // Moving backward (left) in tabs: new content slides in from left
    setSlideDirection(newIndex > oldIndex ? 'right' : 'left')
    setCurrentTab(newTab as TabValue)
  }
    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden">
        <Tabs value={currentTab} onValueChange={handleTabChange} className="space-y-6">
          ...
          {/* Profile Tab */}
          <TabsContent value="profile" slideDirection={slideDirection} className="space-y-6">
          ...
          {/* Accounts Tab */}
          <TabsContent value="accounts" slideDirection={slideDirection} className="space-y-6">
            <Card className="border-border shadow-soft">
              <CardHeader>
          ...
          {/* Security Tab */}
          <TabsContent value="security" slideDirection={slideDirection} className="space-y-6">
            <Card className="border-border shadow-soft">
              <CardHeader>
          ...
          {/* Preferences Tab */}
          <TabsContent value="preferences" slideDirection={slideDirection} className="space-y-6">
            <Card className="border-border shadow-soft">
              <CardHeader>
