...
import { TrendingUp, Target, Shield, PiggyBank, Bell, Zap, ArrowRight, CheckCircle2, Wallet, ArrowUpRight, ArrowDownRight, LayoutDashboard, Receipt, Tag, CreditCard, Settings, DollarSign, Loader2 } from 'lucide-react'
...
export function WaitlistLanding() {
  ...
  useEffect(() => {
    apiClient.get('/health/landing-stats/')
  ...
  const handleJoinWaitlist = async (e: React.FormEvent) => {
    ...
    try {
      await apiClient.post('/marketing/waitlist/', { email })
      setSubmitted(true)
      setEmail('')
    } catch (err: any) {
      console.error(err)
          ...
          <div className="grid gap-12 lg:grid-cols-2 lg:gap-8 items-center">
            ...
            >
                ...
                {isAuthenticated ? (
                  ...
                  <div className="space-y-4">
                    {!submitted ? (
                      <form onSubmit={handleJoinWaitlist} className="flex flex-col sm:flex-row gap-2">
                        <Input
                          ...
                          className="h-12 bg-background/50 backdrop-blur-sm border-primary/20 focus:border-primary text-base"
                        />
                        <Button 
                          type="submit" 
                          size="lg" 
                          disabled={loading}
                          className="rounded-lg h-12 px-8 text-base shadow-xl shadow-primary/20 hover:shadow-primary/30 transition-all duration-300"
                        >
                          {loading ? (
                            <>
                      ...
                      <motion.div 
                        ...
                        className="p-4 rounded-lg bg-green-500/10 border border-green-500/20 text-green-600 dark:text-green-400 flex items-center gap-3"
                      >
                    ...
                    <p className="text-xs text-muted-foreground pl-1">
                      Join {stats.active_users > 0 ? stats.active_users + "+" : "thousands of"} others waiting for access.
                    </p>
                  </div>
                )}
              </motion.div>
              ...
              <motion.div variants={item} className="mt-10 flex items-center gap-4 text-sm text-muted-foreground">
                ...
                <p>Trusted by {stats.active_users > 0 ? stats.active_users.toLocaleString() : "10,000"}+ users</p>
              </motion.div>
            </motion.div>
