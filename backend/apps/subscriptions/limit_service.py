...
class SubscriptionLimitService:
    ...
    @staticmethod
    def is_enabled() -> bool:
        """Check if subscription feature is enabled globally."""
        from django.conf import settings

        return getattr(settings, "ENABLE_SUBSCRIPTION", True)
    ...
    def get_limit(user: User, feature_type: str) -> Optional[int]:
        ...
        """
        # If feature is disabled, return simplified "unlimited" or "allowed" values
        if not SubscriptionLimitService.is_enabled():
            # For boolean features, return True (allowed)
            try:
                if is_boolean_feature(feature_type):
                    return True
            except ValueError:
                pass
            # For numeric features, return None (unlimited)
            return None
        ...
        if not status_info["is_active"] and tier != TIER_FREE:
            tier = TIER_FREE
            logger.info(
                f"Subscription expired for user {user.id}, using free tier limits"
            )
        ...
        try:
            ...
            logger.error(
                f"Error getting limit for user {user.id}, tier {tier}, feature {feature_type}: {e}"
            )
    ...
    def can_access_feature(user: User, feature_type: str) -> bool:
        ...
        if not is_boolean_feature(feature_type):
            raise ValueError(
                f"Feature {feature_type} is not a boolean feature. Use check_limit() for numeric limits."
            )
    ...
    @staticmethod
    def get_usage_info(
        user: User, feature_type: str, get_current_count: Callable[[User], int]
    ) -> dict:
        """
        ...
        limit_value = (
            None
            if is_unlimited_limit
            else (limit.days if isinstance(limit, timedelta) else int(limit))
        )
