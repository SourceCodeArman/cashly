...
def detect_unusual_spending(user):
    ...
    current_spending = (
        Transaction.objects.filter(
            user=user, date__gte=current_month_start, amount__lt=0
        )
        .values("category__name", "category__category_id")
...
def generate_savings_opportunities(user):
    ...
    for cat in category_spending:
        ...
        if amount > 500 and category_name in [
            "Dining",
            "Entertainment",
            "Shopping",
            "Food & Drink",
...
# Removed _get_financial_summary_for_ai as it is now in utils.py
...
def generate_ai_recommendations(user):
    ...
    try:
        # 1. Gather context
        from .utils import get_financial_summary_for_ai

        summary = get_financial_summary_for_ai(user)

        # 2. Get advice from AI
        advisor = AIFinancialAdvisor()
        recommendations = advisor.generate_advice(summary)
        ...
        for rec in recommendations:
            ...
            priority = priority_map.get(
                rec.get("priority", "medium").lower(), InsightPriority.MEDIUM
            )

            insight, created = Insight.objects.update_or_create(
                user=user,
                insight_type=InsightType.AI_RECOMMENDATION,
                title=rec.get("title", "Financial Recommendation"),
                defaults={
...
def generate_insights(user):
        ...
        if item["increase_percentage"] > 50:  # More than 50% increase
            insight, created = Insight.objects.update_or_create(
                ...
                defaults={
                    ...
                    if item["increase_percentage"] > 100
    ...
    ai_insights = generate_ai_recommendations(user)
    created_insights.extend(ai_insights)
