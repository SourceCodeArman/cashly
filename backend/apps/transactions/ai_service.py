...
Rules:
1. If description contains "interest" or "intrst", categorize as "Fees & Charges" or "Bills & Utilities".
2. If merchant or description contains "payment", "pymnt", or "bill":
    - Checks for "thank you" or credit card names (Chase, Amex, Citi, Capital One) -> categorize as "Transfer" or "Credit Card Payment".
    - Otherwise -> prioritize "Bills & Utilities" over "Other".
...
Only respond with valid JSON. Do not include any additional text before or after the JSON."""
    ...
    def categorize_batch(
        ...
        for transaction in transactions_data:
            transaction_id = transaction.get("transaction_id") or transaction.get(
                "id", "unknown"
            )
            ...
            try:
                ...
                logger.error(
                    f"Error categorizing transaction {transaction_id}: {str(e)}"
                )

        return results


class GeminiCategorizationService(AICategorizationService):
    """
    Google Gemini-based AI categorization service.

    Uses Google's Gen AI SDK (google-genai) for transaction categorization.
    """
    ...
    def __init__(self, api_key: str = None, model: str = None):
        ...
        # Fallback for deprecated model in .env
        if "gemini-1.5-flash" in self.model_name:
            logger.warning(
                f"Detected deprecated model '{self.model_name}'. Automatically upgrading to 'gemini-2.0-flash'."
            )
...
"""
    ...
    def categorize_transaction(
        ...
        try:
            ...
            # Call Gemini API using the new Client SDK
            ...
            # The response object structure in the new SDK
    ...
    def _parse_response(self, response_text: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            # Clean up response text if needed (though response_mime_type="application/json" handles most)
    ...
    def categorize_batch(
        ...
        for transaction in transactions_data:
            ...
            try:
                ...
                if result:
                    results[transaction_id] = result
                    # Respect rate limits - brief pause if needed, though usually Gemini is fast enough
            except Exception as e:
                logger.error(
                    f"Error categorizing transaction {transaction_id}: {str(e)}"
                )
...
class AIServiceFactory:
    ...
    def create_service() -> Optional[AICategorizationService]:
        ...
        if provider == "ollama":
            ...
            return GeminiCategorizationService(api_key=api_key, model=model)
