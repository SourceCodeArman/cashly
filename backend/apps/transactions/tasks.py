    ...
    for user in users:
        try:
            result = detect_recurring_transactions(
                user=user, min_occurrences=3, lookback_days=180
            )
            ...
            logger.error(
                f"Error detecting recurring for user {user.id}: {e}", exc_info=True
            )
...
def detect_all_users_transfers():
    ...
    for user in users:
        try:
            result = detect_transfers(user=user, lookback_days=30)
            ...
            logger.error(
                f"Error detecting transfers for user {user.id}: {e}", exc_info=True
            )
    ...
    try:
        ...
        if result and "detected" in result:
            logger.info(
                ...
                f"marked {result.get('updated_count', 0)} transactions"
            )
        else:
            logger.info(f"User {user_id}: No recurring transactions detected")
        return result
        ...
        logger.error(
            f"Error detecting recurring for user {user_id}: {e}", exc_info=True
        )
        return None
...
def detect_user_transfers(user_id):
    ...
    try:
        ...
        logger.error(
            f"Error detecting transfers for user {user_id}: {e}", exc_info=True
        )
        return None
...
def sync_account_transactions(account_id):
    ...
    try:
        ...
        uncategorized = Transaction.objects.filter(
            account=account, category__isnull=True
        )
