...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = (
        LimitOffsetPagination  # Use limit/offset instead of page-based pagination
    )
    filter_backends = [
        DjangoFilterBackend,
    ...
    def get_queryset(self):
        ...
        try:
            ...
            logger.warning(
                f"Error enforcing transaction history limit: {e}", exc_info=True
            )
        ...
        if amount_min or amount_max:
            ...
            queryset = queryset.annotate(abs_amount=Abs("amount"))

            if amount_min:
                queryset = queryset.filter(abs_amount__gte=amount_min)
            if amount_max:
                queryset = queryset.filter(abs_amount__lte=amount_max)
    ...
    def perform_create(self, serializer):
        ...
        if (
            ...
            and getattr(settings, "AI_AUTO_CATEGORIZE_ON_SYNC", True)
            ...
            try:
                ...
                if SubscriptionLimitService.can_access_feature(
                    self.request.user, FEATURE_AI_CATEGORIZATION
                ):
                    suggested_category = auto_categorize_transaction(transaction)

                if suggested_category:
                    apply_category_to_transaction(
                        transaction, suggested_category, user_modified=False
                    )
                    logger.info(
    ...
    def stats(self, request):
        ...
        stats_payload = TransactionStatsSerializer(
            data={
    ...
    def suggest_category(self, request, pk=None):
        ...
        try:
            ...
            SubscriptionLimitService.enforce_limit(
                user=request.user, feature_type=FEATURE_AI_CATEGORIZATION
            )
        except FeatureNotAvailable as e:
            logger.info(
                f"AI categorization feature not available for user {request.user.id}: {e}"
            )
        ...
        if apply:
            try:
                ...
                if apply_category_to_transaction(
                    transaction, category, user_modified=True
                ):
                    return Response(
                        {
                            "status": "success",
                            "data": {
                                "suggestion": CategorySuggestionSerializer(
                                    suggestion
                                ).data,
                                "applied": True,
                                "transaction_id": str(transaction.transaction_id),
    ...
    def bulk_suggest_categories(self, request):
        ...
        transactions = Transaction.objects.filter(
            transaction_id__in=transaction_ids, user=request.user
        )
        ...
        for transaction in transactions:
            suggestions = get_category_suggestions(transaction, limit=1)
            if suggestions:
                results[str(transaction.transaction_id)] = CategorySuggestionSerializer(
                    suggestions[0]
                ).data
            else:
    ...
    def mark_non_recurring(self, request):
        ...
        try:
            ...
            updated_count = Transaction.objects.filter(
                transaction_id__in=transaction_ids, account__user=request.user
            ).update(
                is_recurring=False,
            ...
            logger.error(
                f"Error marking transactions as non-recurring: {e}", exc_info=True
            )
            return Response(
                {
                    "status": "error",
    ...
    def similar_recurring(self, request, pk=None):
        ...
        try:
            ...
            logger.error(
                f"Error finding similar recurring transactions: {e}", exc_info=True
            )
            return Response(
                {
                    "status": "error",
    ...
    def detect_transfers(self, request):
        ...
        try:
            result = detect_transfers(user=request.user, lookback_days=lookback_days)
    ...
    def mark_transfer_pair(self, request, pk=None):
        ...
        try:
            transaction2 = Transaction.objects.get(
                transaction_id=other_id, user=request.user
            )
...
class CategoryViewSet(viewsets.ModelViewSet):
    ...
    pagination_class = (
        None  # Disable pagination for categories - there are limited system categories
    )

    def get_queryset(self):
        """Return system categories and user's custom categories."""
        queryset = Category.objects.for_user(self.request.user).prefetch_related(
            "subcategories"
        )

        # Filter for parent categories only (those without a parent_category)
        parent_only = (
            self.request.query_params.get("parent_only", "false").lower() == "true"
        )
        if parent_only:
    ...
    def create(self, request, *args, **kwargs):
        """Create category and return complete data with ApiResponse contract."""
        # Use the create serializer for validation
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        self.perform_create(serializer)

        # Serialize the created instance with the full CategorySerializer
        # to include all fields like id and is_system_category
        instance = serializer.instance
        output_serializer = CategorySerializer(instance)
    ...
    def update(self, request, *args, **kwargs):
        """Update category and return complete data with ApiResponse contract."""
        ...
        cache_key = f"categories_list_user_{request.user.id}"
        cache.delete(cache_key)

        # Return the complete updated category data
        return Response(
            {
                "status": "success",
                "data": serializer.data,
    ...
    def perform_create(self, serializer):
        """Set user for custom categories and check subscription limits."""
        # Check if user has access to custom categories feature
        try:
            from apps.subscriptions.limit_service import SubscriptionLimitService
            from apps.subscriptions.exceptions import FeatureNotAvailable
            from apps.subscriptions.limits import FEATURE_CUSTOM_CATEGORIES

            SubscriptionLimitService.enforce_limit(
                user=self.request.user, feature_type=FEATURE_CUSTOM_CATEGORIES
            )
        except FeatureNotAvailable as e:
            from rest_framework.exceptions import PermissionDenied

            logger.info(
                f"Custom categories feature not available for user {self.request.user.id}: {e}"
            )
            raise PermissionDenied(
                detail={
                    "status": "error",
                    "message": "Custom categories require a Pro or Premium subscription",
            ...
            logger.error(f"Error checking custom categories access: {e}", exc_info=True)
            raise PermissionDenied(
                detail="An error occurred while checking subscription limits"
            )
        ...
        cache_key = f"categories_list_user_{self.request.user.id}"
        cache.delete(cache_key)
    ...
    @action(detail=True, methods=["patch"], url_path="rules")
    def update_rules(self, request, pk=None):
        """
        PATCH /api/v1/transactions/categories/:id/rules/
        Update categorization rules for a category.
        ...
        # Only allow updating rules for custom categories
        if category.is_system_category:
            return Response(
                {
                    "status": "error",
                    "data": None,
                    "message": "Cannot update rules for system categories",
                },
        ...
        # Check category rules feature access
        try:
            from apps.subscriptions.limit_service import SubscriptionLimitService
            from apps.subscriptions.exceptions import FeatureNotAvailable
            from apps.subscriptions.limits import FEATURE_CATEGORY_RULES

            SubscriptionLimitService.enforce_limit(
                user=request.user, feature_type=FEATURE_CATEGORY_RULES
            )
        except FeatureNotAvailable as e:
            logger.info(
                f"Category rules feature not available for user {request.user.id}: {e}"
            )
            return Response(e.to_dict(), status=e.status_code)
        except Exception as e:
            logger.error(f"Error checking category rules access: {e}", exc_info=True)
            return Response(
                {
                    "status": "error",
                    "data": None,
        ...
        rules_combination = request.data.get("rules_combination", "OR")

        # Validate rules structure (basic validation)
        valid_fields = ["merchant_name", "description", "amount"]
        valid_operators = [
            "contains",
            "equals",
            "starts_with",
                ...
                return Response(
                    {
                        "status": "error",
                        "data": None,
                        "message": "Each rule must have field, operator, and value",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
                ...
                return Response(
                    {
                        "status": "error",
                        "data": None,
                        "message": f"Invalid field: {rule['field']}. Must be one of: {', '.join(valid_fields)}",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
                ...
                return Response(
                    {
                        "status": "error",
                        "data": None,
                        "message": f"Invalid operator: {rule['operator']}. Must be one of: {', '.join(valid_operators)}",
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )

        # Update category rules and combination mode
        category.rules = rules
        category.rules_combination = rules_combination
        category.save()

        # Invalidate the categories cache for this user (important!)
        from django.core.cache import cache
    ...
    def apply_rules(self, request, pk=None):
        """
        POST /api/v1/transactions/categories/:id/apply-rules/
        Apply category rules to uncategorized or existing transactions.

        Optional query parameters:
        - overwrite: If true, recategorize all matching transactions (default: false)
        """
        category = self.get_object()

        # Only allow for custom categories with rules
        if category.is_system_category:
            return Response(
                {
                    "status": "error",
                    "data": None,
                    "message": "Cannot apply rules for system categories",
                },
                status=status.HTTP_403_FORBIDDEN,
            )
            ...
            return Response(
                {
                    "status": "error",
                    "data": None,
                    "message": "Category has no rules defined",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if overwrite:
            ...
            transactions = Transaction.objects.filter(
                user=request.user, category__isnull=True
            )
        ...
        for transaction in transactions:
            ...
            for rule in category.rules:
                ...
                if rule["field"] == "merchant_name":
                    field_value = transaction.merchant_name
                ...
                if operator == "contains":
                    if (
                        ...
                        and rule_value.lower() not in field_value.lower()
                    ):
                        matches_all_rules = False
                        break
                    ...
                    if isinstance(field_value, str):
                        ...
                        if field_value != float(rule_value):
                            matches_all_rules = False
                            break
                elif operator == "starts_with":
                    if isinstance(
                        field_value, str
                    ) and not field_value.lower().startswith(rule_value.lower()):
                        matches_all_rules = False
                        break
                elif operator == "ends_with":
                    if isinstance(
                        field_value, str
                    ) and not field_value.lower().endswith(rule_value.lower()):
                        matches_all_rules = False
                        break
    ...
    def bulk_categorize_from_plaid(self, request):
        ...
        try:
            ...
            return Response(
                {
                    ...
                    "data": {
                        ...
                        "skipped_already_categorized": stats[
            ...
            logger.error(
                f"Error in bulk_categorize_from_plaid: {str(e)}", exc_info=True
            )
            return Response(
                {
                    "status": "error",
    ...
    def bulk_categorize(self, request):
        ...
        try:
            ...
            updated_count = transactions.update(category=category, user_modified=True)
