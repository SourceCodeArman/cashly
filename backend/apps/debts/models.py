...
class DebtAccount(models.Model):
    ...
    name = models.CharField(max_length=200, help_text="Debt name (e.g., Chase Credit Card)")
    ...
    creditor_name = models.CharField(max_length=200, blank=True, help_text="Creditor/lender name")
    account_number_masked = models.CharField(max_length=20, blank=True, help_text="Last 4 digits")
    
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='active')
    
    opened_date = models.DateField(null=True, blank=True, help_text="Date debt was opened")
    target_payoff_date = models.DateField(null=True, blank=True, help_text="Target date to pay off")
    last_payment_date = models.DateField(null=True, blank=True, help_text="Last payment date")
    ...
    def mark_as_paid_off(self):
        ...
        self.save(update_fields=['status', 'current_balance', 'is_active', 'updated_at'])
...
class DebtPayment(models.Model):
    ...
    debt = models.ForeignKey(DebtAccount, on_delete=models.CASCADE, related_name='payments')
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='debt_payments')
    ...
    payment_type = models.CharField(max_length=20, choices=PAYMENT_TYPE_CHOICES, default='minimum')
...
class DebtPayoffStrategy(models.Model):
    ...
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name='debt_strategies')
