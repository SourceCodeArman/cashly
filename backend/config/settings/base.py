...
ALLOWED_HOSTS = config(
    "ALLOWED_HOSTS",
    default="localhost,127.0.0.1,acetometrical-judah-needier.ngrok-free.dev,192.168.1.42",
...
if DB_USER and DB_PASSWORD and DB_HOST and DB_NAME:
    ...
    DATABASE_URL = config("DB_URL", default=None) or config(
    ...
    if DATABASE_URL:
        try:
            ...
            if "nodename" in str(e).lower() or "servname" in str(e).lower():
                ...
                print(
                    "  1. Your Supabase project is PAUSED (most common)",
                    file=sys.stderr,
                )
                ...
                print(
                    "  2. Check if project is paused - if so, click 'Restore Project'",
                    file=sys.stderr,
                )
                print("  3. Wait 2-5 minutes for project to resume", file=sys.stderr)
                print(
                    "  4. Use individual DB_* environment variables instead",
                    file=sys.stderr,
                )
                print("=" * 70 + "\n", file=sys.stderr)
...
PLAID_API_TIMEOUT = config(
    "PLAID_API_TIMEOUT", default=None, cast=lambda v: int(v) if v else None
)
...
PLAID_WEBHOOK_IDEMPOTENCY_TTL = config(
    "PLAID_WEBHOOK_IDEMPOTENCY_TTL", default=300, cast=int
)
...
AI_AUTO_CATEGORIZE_ON_SYNC = config(
    "AI_AUTO_CATEGORIZE_ON_SYNC", default=True, cast=bool
)
...
PLAID_AUTO_CATEGORIZE_ON_SYNC = config(
    "PLAID_AUTO_CATEGORIZE_ON_SYNC", default=True, cast=bool
)
# Allow overwriting existing categories with Plaid categories (default: False, respects user edits)
PLAID_CATEGORIZATION_OVERWRITE_EXISTING = config(
    "PLAID_CATEGORIZATION_OVERWRITE_EXISTING", default=False, cast=bool
)
